...
Also note that you use `==` and not `=`. `==` is to compare, `=` is to _assign_ (to give a value). Also note that we wrote `if my_number == 7` and not `if (my_number == 7)`. You don't need brackets with `if` in Rust.
...
- Write the _pattern_ on the left and use a `=>` fat arrow to say what to do when it matches.
...
- <https://youtu.be/UAymDOpv_us>
- <https://youtu.be/eqysTfiiQZs>
