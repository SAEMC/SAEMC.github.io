...
So those are two reasons for all the different number types in Rust. Here is another reason: `usize` is the size that Rust uses for _indexing_. (Indexing means "which item is first", "which item is second", etc.) `usize` is the best size for indexing because:
