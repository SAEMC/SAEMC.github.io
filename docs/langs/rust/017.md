...
Tuples in Rust use `()`. We have seen many empty tuples already, because _nothing_ in a function actually means an empty tuple:
...
That prints `"two"`, which is what `b` is. This is called _destructuring_. That is because first the variables are inside a structure, but then we made `a`, `b`, and `c` that are not inside a structure.
