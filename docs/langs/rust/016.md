...
- `Vec<Vec<String>>` this is a `Vec` that has `Vec`s of `String`s. Say for example you wanted to save your favourite book as a `Vec<String>`. Then you do it again with another book, and get another `Vec<String>`. To hold both books, you would put them into another `Vec` and that would be a `Vec<Vec<String>>`.
