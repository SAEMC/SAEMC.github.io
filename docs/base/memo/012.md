...
```python title="def Path oepration 함수 → CPU bound 함수()"
from fastapi import FastAPI
...
```python title="async def Path operation 함수 → await run_in_executor(ProcessPoolExecutor, def CPU bound 함수, *args)"
...
```python title="def Path operation 함수 → Blocking I/O 함수()"
from fastapi import FastAPI
...
```python title="async def path operation 함수 → await run_in_executor(threadpoolexecutor, def blocking i/o 함수, *args)"
